...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            ...
            $res = $con->query($sel);
                while ($row=$res->fetch_assoc()) {
                    $selPr = "select * from tbl_food where food_id='" .$row["food_id"]. "'";
            ...
            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assets/Files/Food/<?php echo $rowpr["food_photo"] ?>"
                        />
                </div>
                <div class="product-details">
                    <div class="product-title"><?php echo $rowpr["food_name"] ?></div>
                    <p class="product-description">
                    <?php echo $rowpr["food_details"] ?>
                    </p>
                </div>
                <div class="product-price "	><?php echo $rowpr["food_price"] ?></div>
                <div class="product-quantity">
                <button onclick="decreaseQty(<?php echo $row["cart_id"] ?>)">-</button>
                <input type="number" id="qty_<?php echo $row["cart_id"] ?>" value="<?php echo $row["cart_qty"] ?>" min="1" style="width:40px">
                <button onclick="increaseQty(<?php echo $row["cart_id"] ?>)">+</button>
                </div>
        ...
        $(document).on('change', '.product-quantity input', function() {
            var cartId = this.id.replace('qty_', '');
            var qty = this.value;
            $.ajax({
                url: "../Assets/AjaxPages/AjaxCart.php?action=Update&id=" + cartId + "&qty=" + qty
            });
            updateQuantity(this);
        });
        ...
        function increaseQty(cartId) {
            var qtyInput = document.getElementById('qty_' + cartId);
